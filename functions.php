...
// AJAX FUNCTION TO CHECK AND UPDATE USERS ONE_SIGNAL CF
...
function sendEmail($recepient,$title,$text,$link,$link_title) {
  ...
  ////2do optimize me please!
  // if($post_id === 'starter'){
  //   $post_content = '<html><head></head><body><h3>'.$title.'</h3><p>'.$text.'<a href="'.$link.'">'.$link_title.'</a>".</p></body></html>';
  //   $post_header = 'Новый урок!';
  // } else if ($post_id === 'finish'){
  //   return false;
  // } else if ($post_id === 'greetings'){
  //   return false;
  // } else {
  //   $post_content = '<html><head></head><body><h3>'.$title.'</h3><p>'.$text.' "<a href="' . $link . '">' . get_the_title($post_id) . '</a>".</p></body></html>';
